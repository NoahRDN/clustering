...
if [ -n "$CFG_URL" ]; then
  ...
  echo "[entrypoint] No HAPROXY_DB_CONFIG_URL provided, using local config"
  if [ -f "$LOCAL_CFG" ]; then
    cp "$LOCAL_CFG" "$TARGET_CFG"
  else
    echo "[entrypoint] No local config found at $LOCAL_CFG — exiting"
...
fetch_remote_config() {
  if [ -z "${HAPROXY_CONFIG_URL:-}" ]; then
    echo "[haproxy-db-entrypoint] HAPROXY_CONFIG_URL non défini" >&2
    return 1
  fi
  ...
  if download_to_file "${HAPROXY_CONFIG_URL}" "${tmp_file}"; then
    if haproxy -c -f "${tmp_file}" >/dev/null 2>&1; then
      ...
      echo "[haproxy-db-entrypoint] Remote config invalid" >&2
    fi
  else
...
ensure_remote_config() {
  while ! fetch_remote_config; do
    echo "[haproxy-db-entrypoint] Retry download in 3s..."
    sleep 3
  done
}
