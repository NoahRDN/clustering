...
download_to_file() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    echo "[haproxy-entrypoint] Installing curl to download configs..."
  ...
  echo "[haproxy-entrypoint] No downloader available (curl/wget)" >&2
  return 1
}
...
fetch_remote_config() {
  ...
  local tmp_file="${CONFIG_REMOTE}.tmp"
